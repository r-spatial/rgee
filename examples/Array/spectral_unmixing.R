...
urbanEndmember <- c(88, 42, 48, 38, 86, 115, 59)
vegEndmember <- c(50, 21, 20, 35, 50, 110, 23)
waterEndmember <- c(51, 20, 14, 9, 7, 116, 4)

# Compute the 3x7 pseudo inverse.
endmembers <- ee$Array(list(urbanEndmember, vegEndmember, waterEndmember))
...
colored <- unmixed$
  arrayProject(list(0))$
  arrayFlatten(list(list("urban", "veg", "water")))
Map$setCenter(-98.4, 19, 11)

# Load a hillshade to use as a backdrop.
srtm <- ee$Image("CGIAR/SRTM90_V4")
hillshade <- ee$Algorithms$Terrain(srtm)$select("hillshade")
