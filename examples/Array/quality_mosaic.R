...
qualityMosaic <- function(bands) {
  ...
  best$arrayProject(list(bandAxis))$arrayFlatten(list(bandNames))
}
...
get_ndvi <- function(img) {
  img$normalizedDifference(c("B4", "B3"))$addBands(img)
}

withNd <- l7$map(get_ndvi)
...
rgb <- greenest$select(c("B3", "B2", "B1"))

Map$setCenter(-90.08789, 16.38339, 11)
Map$addLayer(rgb, list(gain = c(1.4, 1.4, 1.1)), "Greenest")
