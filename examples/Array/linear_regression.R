...
# 3. NDVI.
addBand <- function(image) {
  date <- ee$Date(image$get("system:time_start"))
  yearOffset <- date$difference(ee$Date(start), "year")
  ndvi <- image$normalizedDifference(list("B4", "B3"))
  ee$Image(1)$addBands(yearOffset)$addBands(ndvi)$toDouble()
}
...
# NDVI trend.
slope <- fit$arrayGet(c(1, 0))

Map$setCenter(lng, lat, 12)
Map$addLayer(slope, list(min = -0.03, max = 0.03), "Slope")
