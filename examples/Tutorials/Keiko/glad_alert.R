...
geometry <- ee$Geometry$Polygon(
  list(
    c(153.11338711694282, -28.12778417421283),
    c(153.11338711694282, -28.189835226562256),
    c(153.18943310693305, -28.189835226562256),
    c(153.18943310693305, -28.12778417421283)
  )
)

Map$centerObject(ee$FeatureCollection(geometry), zoom = 14)

imageDec <- ee$Image("COPERNICUS/S2_SR/20191202T235239_20191202T235239_T56JNP")
Map$addLayer(
  imageDec,
  list(bands = c("B4", "B3", "B2"), min = 0, max = 1800),
  "True colours (Dec 2019)"
)

Map$addLayer(
  imageDec,
  list(bands = c("B3", "B3", "B3"), min = 0, max = 1800),
  "grey"
)
...
# Export the image
imageMosaic <- ee$ImageCollection(c(
