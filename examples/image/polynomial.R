...
img <- ee$Image("MODIS/006/MOD09GA/2012_03_09")$
  select(c("sur_refl_b01", "sur_refl_b04", "sur_refl_b03"))$
  multiply(0.0001)

# Apply the polynomial enhancement.
adj <- img$polynomial(c(-0.2, 2.4, -1.2))

Map$setCenter(-107.24304, 35.78663, 8)
Map$addLayer(img, list(min = 0, max = 1), "original") +
Map$addLayer(adj, list(min = 0, max = 1), "adjusted")
