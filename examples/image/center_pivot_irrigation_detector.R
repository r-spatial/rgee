...
ndvi_map <- Map$addLayer(
  eeObject = ndvi,
  visParams = list(min = 0, max = 1, palette = palette),
  name = "Landsat NDVI"
)
ndvi_map
...
circle_kernels_01 <- Map$addLayer(
  eeObject = peaks$updateMask(peaks),
  visParams = list(palette = "FF3737"),
  name = "Kernel Peaks"
)
circle_kernels_01
...
circle_kernels_02 <- Map$addLayer(centers$updateMask(centers), list("palette" = "4285FF"), "Ring centers")
circle_kernels_02
ndvi_map + circle_kernels_01 + circle_kernels_02
