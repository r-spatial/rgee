...
#' Create an animated video thumbnail from an Earth Engine ImageCollection
#'
#' Create an animated video thumbnail (it's a magick-image object) from a EE
#' ImageCollection. Note: Animations can only be created when the image
#' visualization creates an RGB or RGBA image. This can be done by mapping
#' a visualization onto an ImageCollection (\code{ic$map(function(img) img$visualize(...))})
#' or specifying three bands in parameters argument (See examples). This function is a
...
#' @param quiet Logical. Suppress info message.
#' @details
#' The parameters argument are identical to visParams (See \code{rgee::Map$addLayer}),
#' plus, optionally:
#' \itemize{
...
#' )
#'
#' ## 1.2 Download and display the GIF!
...
ee_utils_gif_creator <- function(ic, parameters, quiet = FALSE) {
  ...
  download.file(animation_url, temp_gif)
  image_read(temp_gif)
}


# ee_utils_add_text_to_gif <- function(image,
#                                      format,
#                                      text_sequence,
#                                      xy = c(10, 10),
#                                      font_type = "arial.ttf",
