...
ee_html_usage_rstudio <- function(pydoc){
  ...
  to_display <- name_strong %>%
    paste0(regmatches(pydoc$signature, extract_parenthesis_text)[[1]])
  sprintf('<h3 style="%s">Usage</h3><p style = "%s">%s</p>',ee_css_h3_rstudio(),ee_css_pre_simple(),to_display)
}
...
ee_html_arguments_rstudio <- function(parameters) {
  ...
  p_style <- 'display: block;margin-block-start: 1em;margin-block-end: 1em;margin-inline-start: 0px;margin-inline-end: 0px;'
  table_style <- 'display: table;border-collapse: separate;border-spacing: 10px;border-color: grey;'
  td_style <- 'display: table-cell;vertical-align: inherit;font-family: sans-serif;font-size: 10pt;'
  table_head <- sprintf('<table summary="R argblock" style="%s">',table_style)
  table_end <- '</table>'
  rows <- ""
  for (index in seq_along(parameters)) {
  ...
  arguments_table <- paste0(table_head,rows,table_end)
  sprintf('<h3 style="%s">Arguments</h3>%s',ee_css_h3_rstudio(), arguments_table)
}
...
ee_html_details_rstudio <- function(details){
  ...
  if (nchar(details) < 1) {
    ...
    p_style <- 'font-family: sans-serif; font-size: 10pt;'
    sprintf('<h3 style="%s">Details</h3><p style="%s">%s<p>',ee_css_h3_rstudio(), p_style, details)
  }
}
...
ee_html_returns_rstudio <- function(returns) {
  ...
  if (nchar(returns) < 1 ) {
    ...
    p_style <- 'font-family: sans-serif; font-size: 10pt;'
    sprintf('<h3 style="%s">Returns</h3><p style="%s">%s<p>',ee_css_h3_rstudio(),p_style, returns)
  }
}
...
ee_css_h3_rstudio <- function(){
  paste0('background: white;color: rgb(35%, 35%, 35%);font-family: sans-serif',
         ';font-size: 15px;')
}
