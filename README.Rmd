...
**rgee** provides functions and classes to conveniently interact with Google Earth Engine since R. The main user relevant functions are:

* `ee`: The [EE Python library](https://github.com/google/earthengine-api), use `$` instead of `.`.
* `ee_initialize`: Authenticate and Initialize the Earth Engine and Google Drive API.
* `ee_map`: Display a  `ee$Image`, `ee$Feature`, `ee$FeatureCollection`, or `ee$ImageCollection`
through the [`mapview`](https://github.com/r-spatial/mapview) R package.
* `ee_download_*`: Download the results of an "COMPLETED" Earth Engine task.
...
It put together the main ideas behind of [geeup](https://github.com/samapriya/geeup), [geextract](https://github.com/loicdtx/landsat-extract-gee) and [cartoee](https://github.com/KMarkert/cartoee).
